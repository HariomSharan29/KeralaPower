...
public class ShuntCapacitorDialog extends Dialog {
    ...
    public ShuntCapacitorDialog(@NonNull Context context, JsonObject jsonObject) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.breakerBtn.setText("Capacitor");
        ...
        binding.shuntCapacitorInfoLayout.setVisibility(View.VISIBLE);
        binding.editBreakerLayout.setVisibility(View.GONE);
        binding.headerTitle.setText("Shunt Capacitor");
        ...
        binding.cableBtn.setOnClickListener(view -> {
            ...
            binding.shuntCapacitorInfoLayout.setVisibility(View.GONE);
            binding.nodeInfoLayout.setVisibility(View.GONE);
    ...
    private void getCapacitorInfo() {
        ...
        if (prefManager.getUserType() != null) {
            ...
            Call<ShuntCapacitor> call = apiInterface.getShuntCapacitorData("Bearer " + prefManager.getAccessToken(), jsonObject);
                    ...
                    if (response.code() == 200) {
                        try {
                            ...
                            if (shuntCapacitor.getOutput() != null) {
                                if (shuntCapacitor.getOutput().getDeviceTypeLine() == 1) {
                                    binding.cableBtn.setText("Cable");
                                } else if (shuntCapacitor.getOutput().getDeviceTypeLine() == 2) {
                                    binding.cableBtn.setText("Balance");
                                } else if (shuntCapacitor.getOutput().getDeviceTypeLine() == 17) {
                                    binding.cableBtn.setText("UnBalance");
                                } else {
                                    binding.cableBtn.setText("");
                                }

                                if (shuntCapacitor.getOutput().getSectionId() != null && shuntCapacitor.getOutput().getDeviceTypeLine() != null && prefManager.getUserType() != null) {
                                    requestObject.addProperty("DeviceNumber", shuntCapacitor.getOutput().getLineDeviceNumber());
                                    requestObject.addProperty("DeviceType", shuntCapacitor.getOutput().getDeviceTypeLine().toString());
                                    requestObject.addProperty("UserType", prefManager.getUserType());
                                    requestObject.addProperty("CYMDBNET", prefManager.getDBName());
                                }

                                if (!shuntCapacitor.getOutput().getSectionId().isEmpty() && !shuntCapacitor.getOutput().getSectionId().equals("null") && shuntCapacitor.getOutput().getSectionId() != null) {
                                    binding.sectionTv.setText(shuntCapacitor.getOutput().getSectionId());
                                }
                                ...
                                if (shuntCapacitor.getOutput().getZoneId() != null) {
                                    binding.zoneTv.setText(shuntCapacitor.getOutput().getZoneId());
                                }

                                if (!shuntCapacitor.getOutput().getEquipmentId().isEmpty() && !shuntCapacitor.getOutput().getEquipmentId().equals("null") && shuntCapacitor.getOutput().getEquipmentId() != null) {
                                    binding.shuntTyeTv.setText(shuntCapacitor.getOutput().getEquipmentId());
                                }
                                ...
                                if (shuntCapacitor.getOutput().getStatus() != null) {
                                    if (shuntCapacitor.getOutput().getStatus() == 0) {
                                        binding.shuntStatusTv.setText("Connected");
                                    } else if (shuntCapacitor.getOutput().getStatus() == 1) {
                                        binding.shuntStatusTv.setText("DisConnected");
                                    } else {
                                        binding.shuntStatusTv.setText("By Passed");
                                    }
                                }

                                if (shuntCapacitor.getOutput().getLocation() != null) {
                                    if (shuntCapacitor.getOutput().getLocation() == 1) {
                                        binding.shuntLengthTv.setText("At From Node");
                                    } else if (shuntCapacitor.getOutput().getLocation() == 2) {
                                        binding.shuntLengthTv.setText("At To Node");
                                    } else {
                                        binding.shuntLengthTv.setText("At Middle Node");
                                    }
                                }
                                ...
                                if (!shuntCapacitor.getOutput().getFromNodeId().isEmpty() && !shuntCapacitor.getOutput().getFromNodeId().equals("null") && shuntCapacitor.getOutput().getFromNodeId() != null) {
                                    binding.idFromNodesTv.setText(shuntCapacitor.getOutput().getFromNodeId());
                                }

                                if (!shuntCapacitor.getOutput().getToNodeId().isEmpty() && !shuntCapacitor.getOutput().getToNodeId().equals("null") && shuntCapacitor.getOutput().getToNodeId() != null) {
                                    binding.idToNodeTv.setText(shuntCapacitor.getOutput().getToNodeId());
                                }

                                if (!shuntCapacitor.getOutput().getToNodeId().equals("null") || !shuntCapacitor.getOutput().getFromNodeId().equals("null")) {
